...
class ControllerExtensionModuleEasyPhoto extends Controller {
	...
	public function upload() {
		if(isset($this->request->files["easyphoto"]["name"])){
				...
				if (!is_dir($this->getDirectory(1) . $this->generateModel() . "/")) { //если нет tmp директории
					mkdir($this->getDirectory(1) . $this->generateModel() . "/", 0777, true);
				}
		 		move_uploaded_file($this->request->files["easyphoto"]["tmp_name"], $this->getDirectory(1) . $this->generateModel() . "/" . $this->request->files["easyphoto"]["name"]);
		 }
	}
	...
	public function resize_rename($from_model = array()) {
		...
		$directory = $this->getDirectory() . $this->generateModel() . '/';
		...
		if(isset($this->request->get['product_id']) or isset($from_model['product_id'])){
			...
			$product_model_query = $this->db->query("SELECT model FROM " . DB_PREFIX . "product WHERE product_id = '" . $product_id . "'");
			
			if($product_model_query->row){
				$model = $product_model_query->row['model'];
			}
			...
			$photo_dir_id = $this->getDirectory(1) . $model;
			...
			if(!$from_model){ //прямая загрузка
				$old_file = $this->getDirectory(1) . $this->generateModel() . "/" . $photo;
			}else{
	...
	public function clear_tmp(){
		$dir = DIR_IMAGE . $this->getDirectory() . '/';
		if (file_exists($dir)){
			foreach(glob($dir . '*') as $file){
				unlink($file);
			}
